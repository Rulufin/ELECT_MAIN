...
from utils.discord_tasks.errors import (
    # 既存
    is_rate_limited,
    extract_retry_after,
...
class DiscordQueueManager:
    ...
    async def enqueue(
        ...
        *,
        ...
        rate limit UX:
        - route: 例 "PATCH /channels/{channel_id}"
        - on_rate_limit: retry_after が長い場合に呼ぶ通知コールバック（タスク中1回だけ）
        - notify_threshold: この秒数以上なら通知対象
        """
        if not self._running:
            await self.start()
    ...
    async def _execute_with_retries(
        ...
        *,
        ...
        """
        Important: do not "drop" the task. Either set_result or set_exception.

        429:
          - gate を set
          - retry_after が長い場合だけ on_rate_limit を 1回呼ぶ
          - sleep(retry_after) して retry
        """
        attempt = 0
        notified = False  # タスク中に1回だけ通知
        ...
        while True:
            ...
            try:
                ...
                # Rate limit -> wait and retry (same task)
                if is_rate_limited(e):
                    ...
                    # ---- 短い待機は retry 回数が残ってる時だけ sleep→retry ----
                    if attempt <= max_retries:
                        await asyncio.sleep(retry_after)
                        continue


                # transient 5xx/timeouts -> retry a few times
                if is_transient_http_error(e) and attempt <= max_retries:
