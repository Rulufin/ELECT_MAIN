...
from utils.discord_tasks.buckets import (
    bucket_webhook_followup,
    bucket_webhook_edit_original,
    bucket_webhook_delete_original,
)
from utils.discord_tasks.queued_call import queued_call
from utils.discord_tasks._missing import opt, opt_list
...
async def safe_defer(
    ...
    *,
    ...
    # defer は webhook ではないが、最初のACKなのでここは直呼びでOK（3秒制限優先）
...
async def safe_edit_original_response(
    ...
    *,
    ...
    msg = await queued_call(_run, bucket=pair)
