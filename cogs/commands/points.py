...
from services.points.ui.views import Point_Manage_Panel_View
from services.points.enums import Points_Type, Genre_Type
from services.points.post.service import PostPointService
from services.points.post.configs import POST_POINT_RULES

from utils.discord.helpers.user_ids import coerce_user_ids, coerce_user_ids_or_raw_id
...
class Points_Main_Cog(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        self.post_point_service = PostPointService(fs_points=self.fs_points)
    ...
    @app_commands.command(name="delete_pic_point", description="写真投稿チャンネルで誤付与されたポイントをすべて削除します")
    @app_commands.guild_only()
    @app_commands.default_permissions(administrator=True)
    async def delete_pic_point_command(self, interaction: Interaction):
        await interaction.response.defer(ephemeral=True)

        channel_ids = list(POST_POINT_RULES.keys())
        affected = await self.post_point_service.cleanup_by_channel_ids(channel_ids)

        total_deleted = sum(affected.values())
        user_count = len(affected)

        await interaction.followup.send(
            f"削除完了: {user_count} ユーザー / {total_deleted} 件のポイントイベントを削除しました。",
            ephemeral=True,
        )
    ...
    @app_commands.guild_only()
    @app_commands.default_permissions(administrator=True)
    @app_commands.describe(limit="チャンネルごとに遡るメッセージ数（デフォルト: 1000）")
    async def set_pic_point_command(self, interaction: Interaction, limit: app_commands.Range[int, 1, 10000] = 1000):
        await interaction.response.defer(ephemeral=True)
        ...
        for channel_id, rule in POST_POINT_RULES.items():
            channel = guild.get_channel(channel_id)
            if channel is None:
                lines.append(f"- `{channel_id}`: チャンネルが見つかりません")
                continue
            ...
            async for message in channel.history(limit=limit, oldest_first=True):
                ...
                existing = await self.post_point_service._get_message_index(message.id)
            ...
            lines.append(f"- {channel.mention} (`{rule.note}`): 付与 {granted} 件 / スキップ {skipped} 件")

        summary = "\n".join(lines)
        await interaction.followup.send(
            f"精査完了\n{summary}\n\n合計: 付与 **{total_granted}** 件 / スキップ {total_skipped} 件",
            ephemeral=True,
        )
